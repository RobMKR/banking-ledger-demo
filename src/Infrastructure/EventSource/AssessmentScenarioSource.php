...
namespace Ledger\Infrastructure\EventSource;
...
/**
 * The brief's ten events and two accounts, transcribed once.
 *
 * Concrete, with no interface in front of it. There is one event source and nothing injects a
 * different one, so a port here would be a seam with nothing to pass through it — the same
 * reasoning that keeps the rules concrete. The presenter is the one place two implementations
 * genuinely exist, and that is where the interface lives.
 */
final class AssessmentScenarioSource
{
    ...
    public static function stream(): EventStream
    {
    ...
    /**
     * The same ten events, each emitted twice — a retry or an operator re-run.
     *
     * The repeats are freshly constructed rather than the same object twice, so a guard that
     * deduplicated on object identity would still pass every other test and fail against this.
     */
    public static function streamWithDuplicates(): EventStream
    {
        $first = self::stream()->asListed();
        $again = self::stream()->asListed();
